...
const testResourceNameDynamicHostVolumeRegister = "nomad_dynamic_host_volume_register.test"

func TestResourceDynamicHostVolumeRegister_import(t *testing.T) {
	...
	resource.Test(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			{
				Config: testResourceDynamicHostVolumeRegister_config(name, nodeID),
				Check: testResourceDynamicHostVolumeRegister_check(
...
func TestResourceDynamicHostVolumeRegister_refresh(t *testing.T) {
	name := acctest.RandomWithPrefix("tf-nomad-test")
	nodeID := testResourceDynamicHostVolume_getNodeID(t)
...
func TestResourceDynamicHostVolumeRegister_update(t *testing.T) {
	name := acctest.RandomWithPrefix("tf-nomad-test")
	nodeID := testResourceDynamicHostVolume_getNodeID(t)
	...
	resource.Test(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			...
			{
				Config: testResourceDynamicHostVolumeRegister_update(name, nodeID),
				Check: testResourceDynamicHostVolumeRegister_update_check(
					testResourceNameDynamicHostVolumeRegister, name),
			},
		},
		CheckDestroy: testResourceDynamicHostVolume_checkDestroy(
...
func testResourceDynamicHostVolumeRegister_config(name, nodeID string) string {
	return fmt.Sprintf(`
resource "nomad_dynamic_host_volume_register" "test" {
  name      = "%s"
  capacity  = "12 GiB"
  node_id   = "%s"
...
func testResourceDynamicHostVolumeRegister_update(name, nodeID string) string {
	return fmt.Sprintf(`
resource "nomad_dynamic_host_volume_register" "test" {
  name      = "%s"
  capacity  = "15 GiB"
...
func testResourceDynamicHostVolumeRegister_check(resourceName, name string) resource.TestCheckFunc {
...
func testResourceDynamicHostVolumeRegister_update_check(resourceName, name string) resource.TestCheckFunc {
	return func(s *terraform.State) error {
		client := testProvider.Meta().(ProviderConfig).client
		id, err := testResourceDynamicHostVolume_getID(s, resourceName)
		if err != nil {
